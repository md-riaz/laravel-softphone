...
<div class="page-header">
    <div class="page-header-row">
        <h1 class="page-title">Create PBX Connection</h1>
    </div>
</div>
        ...
        <form method="POST" action="{{ route('admin.pbx-connections.store') }}">
            ...
            <div class="form-row">
                <div class="form-group" style="flex:2;">
                    ...
                    <input type="text" class="form-input @error('host') is-invalid @enderror" id="host" name="host" value="{{ old('host') }}" placeholder="pbx.example.com" required>
                ...
                <div class="form-group" style="flex:1;">
                    ...
                    <input type="number" class="form-input @error('port') is-invalid @enderror" id="port" name="port" value="{{ old('port', 5060) }}" required>
                    @error('port')<p class="form-error">{{ $message }}</p>@enderror
                </div>
            </div>
            <div class="form-group">
                <label for="wss_url" class="form-label">WebSocket URL</label>
                <input type="text" class="form-input @error('wss_url') is-invalid @enderror" id="wss_url" name="wss_url" value="{{ old('wss_url') }}" placeholder="wss://pbx.example.com:8089/ws">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-input @error('stun_server') is-invalid @enderror" id="stun_server" name="stun_server" value="{{ old('stun_server') }}" placeholder="stun:stun.example.com:3478">
                @error('stun_server')<p class="form-error">{{ $message }}</p>@enderror
            </div>
            <div class="form-group">
                <label for="turn_server" class="form-label">TURN Server</label>
                <input type="text" class="form-input @error('turn_server') is-invalid @enderror" id="turn_server" name="turn_server" value="{{ old('turn_server') }}" placeholder="turn:turn.example.com:3478">
                @error('turn_server')<p class="form-error">{{ $message }}</p>@enderror
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label for="turn_username" class="form-label">TURN Username</label>
                    <input type="text" class="form-input @error('turn_username') is-invalid @enderror" id="turn_username" name="turn_username" value="{{ old('turn_username') }}">
                    @error('turn_username')<p class="form-error">{{ $message }}</p>@enderror
                </div>
                <div class="form-group">
                    <label for="turn_password" class="form-label">TURN Password</label>
                    <input type="password" class="form-input @error('turn_password') is-invalid @enderror" id="turn_password" name="turn_password">
